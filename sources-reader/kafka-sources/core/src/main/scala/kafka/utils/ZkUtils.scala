 ...
 * LeaderChangeListener、PartitionModificationsListener、PreferredReplicaElectionListener、
 * PartitionsReassignedListener、ReassignedPartitionsIsrChangeListener
 *
 * Kafka中提供了四个IZkChildListener接口的实现：
 * DeleteTopicsListener、TopicChangeListener、IsrChangeNotificationListener、BrokerChangeListener
 *
 * Kafka中提供了一个IZkStateListener接口的实现：
 * SessionExpirationListener
 *
 * TopicChangeListener负责管理Topic的增删，监听“/brokers/topics”节点的子节点变化，会为每个新增的Topic注册一个PartitionModificationsListener，
 * 然后调用onNewPartitionCreation()方法完成新增Topic的分区状态以及副本状态转换
 *
 * DeleteTopicsListener会监听ZooKeeper中“/admin/delete_topics”节点下的子节点变化，在该路径下添加需要被删除的Topic时，
 * DeleteTopicsListener会被触发，它会将该待删除的Topic交由TopicDeletionManager执行Topic删除操作。
 *
 * 在新增Topic时会为每个Topic注册一个PartitionModificationsListener，在成功删除Topic之后会将注册的PartitionModificationsListener删除
 * PartitionModificationsListener 会监听“/brokers/topics/[topic_name]”节点中的数据变化，主要用于监听一个Topic的分区变化。
 *
 * BrokerChangeListener 是ReplicaStateMachine中唯一的ZooKeeper Listener，它会监听“/brokers/ids”节点下的子节点变化，
 * 主要负责处理Broker的上线和故障下线。当Broker上线时会在“/brokers/ids”下创建临时节点，下线时会删除对应的临时节点。
 *
 * IsrChangeNotificationListener
 * Follower副本会与Leader副本会进行消息同步，当Follower副本追上Leader副本时会被添加到ISR集合中，当Follower副本与Leader副本差距太大时会被踢出ISR集合。
 * Leader副本不仅会在ISR集合变化时将其记录到ZooKeeper中，还会调用ReplicaManager.recordIsrChange()方法记录到isrChangeSet集合中，
 * 之后通过isr-change-propagation定时任务将该集合中周期性地写入到ZooKeeper的“/isr_change_notification”路径下。
 * IsrChangeNotificationListener用于监听此路径下的子节点变化，当某些分区的ISR集合变化时通知整个集群中的所有Broker。
 *
 * PreferredReplicaElectionListener负责监听的ZooKeeper节点是“/admin/preferred_replica_election”。
 * 当管理人员通过PreferredReplicaLeaderElectionCommand命令指定某些分区需要进行“优先副本”选举时会将指定分区的信息写入该节点，从而触发PreferredReplicaElectionListener进行处理。
