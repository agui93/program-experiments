...
class ControllerContext(val zkUtils: ZkUtils) {
  ...
  //Controller的年代信息，初始为0。Controller的年代信息存储的ZK路径是“/controller_epoch”。每次重新选举新的Leader Controller，epoch字段值就会增加1
  ...
  //记录了正在重新分配副本的分区,ReassignedPartitionsContext封装了新分配的AR集合信息以及用于监听ISR集合变化的ReassignedPa rtitionsIsrChangeListener
...
class KafkaController(val config: KafkaConfig, zkUtils: ZkUtils, val brokerState: BrokerState, time: Time, metrics: Metrics, threadNamePrefix: Option[String] = None) extends Logging with KafkaMetricsGroup {
  ...
  //KafkaController的上下文信息，缓存了ZooKeeper中记录的整个集群的元信息，例如，可用Broker、全部的Topic、分区、副本的信息
  val controllerContext = new ControllerContext(zkUtils)

  //用于管理集群中所有Partition状态的状态机
